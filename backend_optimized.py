...
# Render环境优化配置
if IS_RENDER:
    logger.info("检测到Render环境，启用优化模式")
    # 减少历史数据天数以节省内存
    Config.HISTORICAL_DAYS = min(Config.HISTORICAL_DAYS, 90)
    # 增加更新间隔以减少CPU使用
    Config.UPDATE_INTERVAL_HOURS = max(Config.UPDATE_INTERVAL_HOURS, 2)
    logger.info(f"优化配置: HISTORICAL_DAYS={Config.HISTORICAL_DAYS}, UPDATE_INTERVAL_HOURS={Config.UPDATE_INTERVAL_HOURS}")
...
if __name__ == '__main__':
    ...
    print(f"环境: {'Render (优化模式)' if IS_RENDER else '本地开发'}")
    print(f"API地址: http://0.0.0.0:{os.environ.get('PORT', 5000)}")
    print("=" * 60)
    
    # Render环境使用gunicorn，本地开发使用Flask开发服务器
    if IS_RENDER:
        # Render环境由gunicorn启动，不需要这里启动服务器
        pass
    else:
        # 本地开发环境
